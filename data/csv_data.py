...
def log_contract_start(S_SESSION):
    ...
    contract_cache[contract_id] = {
        ...
        'avg_change':                   S_SESSION.avg_change,
        'percentageChange':             S_SESSION.current_candle_PercentageChange,
        'spread':                       S_SESSION.spread,
        'std_dev':                      S_SESSION.std_dev,
...
def append_contract_to_csv(contract_id):
    ...
    with open(source_data_file, mode='a', newline='') as file:
        ...
        writer.writerow([
            ...
            contract_data['avg_change'],
            contract_data['percentageChange'],
            contract_data['spread'],
            contract_data['std_dev'],
    ...
    with open(source_data_file, mode='r') as file:
        ...
        for row in reader:
            if row[0] == contract_id:  # Update the row with the matching contract ID
                row[25] = contract_cache[contract_id]['Status']  # Update Status
            rows.append(row)
