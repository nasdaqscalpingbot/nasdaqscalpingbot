...
csv_filename = "nasdaq_backtest.csv"
...
# List of known US stock market holidays in 2024
MARKET_HOLIDAYS = {
    ...
    while start_date <= end_date:
        ...
        if start_date.weekday() >= 5 or date_str in MARKET_HOLIDAYS:
            ...
            candle_history = make_request(
                "GET",
                f"/api/v1/prices/US100?resolution=MINUTE_5&max=1000&from={date_str}T10:00:00&to={date_str}T22:00:00"
            )
...
def fetch_and_convert_to_3min_candles(candle_history):
    ...
    with open(csv_filename, mode="w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["Time", "Open", "Close", "High", "Low", "Volume"])  # Header row
...
def backtest_strategy():
    """Backtest the strategy on the full dataset and save 'good' trades for AI training."""
    ...
    # Track performance
    results = {"bot_macd": {"total": 0, "correct": 0, "rejected": 0, "rejected_due_to_market": 0, "profit": 0}}
    ...
    with open(training_data_file, mode="a", newline="") as file:
        ...
        if not file_exists:
            writer.writerow([
                "Timestamp", "Open", "Close", "High", "Low", "ATR",
                "MACD", "Signal Line", "3-Candle Movement", "Volatility", "Trade"
            ])
        ...
        for i in range(26, len(historical_candles)):
            ...
            three_candle_signal = three_candle_movement(lst_fetched_candles)
            ...
            if strategy_signal_bot != "HOLD":
                results["bot_macd"]["total"] += 1  # ✅ Count total trades
                current_candle = historical_candles[i]

                # ✅ Ensure we have at least 10 future candles; otherwise, use what's left
                if i + 10 < len(historical_candles):
                    next_10_candles = historical_candles[i + 1:i + 11]  # Take exactly 10 candles
                else:
                    next_10_candles = historical_candles[i + 1:]  # Take remaining candles
                ...
                # ✅ Save the trade to the training dataset
                writer.writerow([
                    i,  # Timestamp (using index for now, replace with real time if available)
                    current_candle[0], current_candle[1], current_candle[2], current_candle[3],
                    atr, bot_macd, bot_signal, three_candle_signal, volatility, strategy_signal_bot
                ])
...
def simulate_trailing_stop_loss(lst_fetched_candles, entry_price, trade_type, candles, min_hold_time=10, contract_size=1):
    ...
    for i, candle in enumerate(candles):
        high, low = candle[3], candle[4]  # High and low prices
        trade_duration += 1
